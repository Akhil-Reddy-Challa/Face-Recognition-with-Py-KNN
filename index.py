...
def train(train_dir, model_save_path = "", n_neighbors = None, knn_algo = 'ball_tree', verbose=False):
    ...
    for class_dir in listdir(train_dir):
        ...
        for img_path in image_files_in_folder(join(train_dir, class_dir)):
            print("Training on ",img_path.strip("knn_examples/"))
...
def draw_preds(img_path, preds):
    ...
    for pred in preds:
        ...
        if name!="N/A":
            print('{',name,end=' } ')
            main_path="/home/deeplearning/Desktop/Mini-Project/sorted_images/"
            if not os.path.exists(main_path+name):
                os.mkdir(main_path+name)
            shutil.copy(img_path,main_path+name)
            source_img.show()
    print()

if __name__ == "__main__":
    knn_clf = train("knn_examples/Training-Images")
    print("----------------------------------- TRAINING COMPLETED -----------------------------------")
    if not os.path.exists("/home/deeplearning/Desktop/Mini-Project/sorted_images"):
        os.mkdir("sorted_images")
    for img_path in listdir("knn_examples/test"):
        print("Looking for Faces in",img_path.strip("knn_examples/test"))
